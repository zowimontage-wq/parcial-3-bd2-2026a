...
def index():
    ...
    cartelera = db.query(
        ...
        SELECT p.id_pelicula, p.titulo, p.clasificacion, p.duracion_min,
               p.poster_url, p.generos_extra, g.nombre AS genero
        FROM pelicula p
...
def _guardar_pelicula(id=None):
    generos_extra = ','.join(request.form.getlist("generos_extra"))
    datos = (
    ...
    if id:
        ...
        db.execute(
            """INSERT INTO pelicula (titulo, id_genero, duracion_min, clasificacion,
               sinopsis, poster_url, activa, generos_extra) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)""",
            datos,
        )
...
# ============================================================
#  Funciones (RF3) + Venta de boletos (RF4, RF5)
# ============================================================
@app.route("/funciones")
def funciones_list():
    funciones = db.query(
...
def funcion_venta(id):
    funcion = db.query(
        """
        SELECT f.*, p.titulo, p.id_pelicula, s.nombre AS sala
        FROM funcion f
        JOIN pelicula p ON p.id_pelicula = f.id_pelicula
        JOIN sala s ON s.id_sala = f.id_sala
    ...
    otras_horas = db.query(
        """
        SELECT f.id_funcion, f.hora, f.fecha, s.nombre AS sala
        FROM funcion f
        JOIN sala s ON s.id_sala = f.id_sala
    ...
    return render_template("funciones/venta.html", funcion=funcion, filas=filas,
                           clientes=clientes, otras_horas=otras_horas)
